...
| 前置导航 | `nm pipeline evaluate <id>` | `evaluation.yaml` |
...
| 作品画像 | `nm pipeline profile <id>` | `work_profile.yaml` |
| 精调 | `nm pipeline refine <id>` | 统计与结构角色推断 |
| 断点续传 | `nm pipeline continue <id>` | 自动继续未完成阶段 |
| 运行报告 | `nm pipeline report <id>` | `reports/latest.md`、`reports/latest.yaml` |
| 产物审计 | `nm validate artifacts <id>` | 只读质量问题列表 |
...
`full` 入库前会检查同名小说；发现重复时默认不重新分析，避免误触发重复 LLM 消耗。
...
| `standard` | 默认无人值守流程 | 默认只分析开头 100 章 core insights |
