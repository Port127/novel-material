...
```text
               ...
               | novels/{id}/stats.md          |  可视化报告（Mermaid）
               | novels/{id}/stats.html        |  交互报告（ECharts+关系图谱）
               +-------------------------------+
```
...
支持的流程模式：
- `full`: material-add → source-format → outline → worldbuilding → characters → tags → scenes → build-index → refine → novel-stats
- `quick`: material-add → source-format → outline → worldbuilding → characters
- `continue`: 从中断点恢复
- `stage`: 执行指定阶段
...
novel-outline         outlined: 生成大纲骨架
       ↓
novel-worldbuilding   outlined: 提取世界观设定（力量体系/地理/势力/背景）
       ↓
novel-characters      outlined: 生成人物体系（+原型+叙事功能）
       ↓
novel-tags            tagged: 生成小说级标签（+套路识别）
       ↓
novel-scenes          complete: 逐章拆分场景+打标签
       ↓
build-index           complete: 构建倒排索引+场景清单
       ↓
refine                refined: 精调大纲/世界观/人物/标签（基于场景数据）
       ↓
novel-stats           refined: 生成统计报告+可视化+交互HTML+关系图谱
       ↓
