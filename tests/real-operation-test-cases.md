...
> 模拟真实用户在日常使用中可能遇到的误操作、异常场景、边界情况。
> 每个案例都是"某个人在某天真的会这么干"的场景。

---

## 一、文件误删与恢复（TC-RO-001 ~ TC-RO-040）

### 1.1 场景 YAML 误删（TC-RO-001 ~ TC-RO-010）

| ID | 操作场景 | 操作步骤 | 预期恢复方式 |
|----|---------|---------|-------------|
| TC-RO-001 | 手动删了一个场景文件 ch0015_s2.yaml | `rm data/novels/nm_xxx/scenes/ch0015_s2.yaml` | git checkout 恢复，或从 SQLite 导出数据手工重建 |
| TC-RO-002 | 误删了整个 scenes/ 文件夹 | `rm -rf data/novels/nm_xxx/scenes/` | git checkout 恢复整个文件夹，或从备份恢复 |
| TC-RO-003 | 用 Finder/资源管理器拖动 scenes 文件夹到回收站 | GUI 操作删除 | 从回收站恢复，然后运行 build-index 重建索引 |
| TC-RO-004 | 清理磁盘空间时误删了某本小说的文件夹 | `rm -rf data/novels/nm_novel_20260405_s4t1/` | git checkout 恢复，同时需要检查 index.yaml 是否仍有该条目 |
| TC-RO-005 | 删了场景文件但 SQLite 中还有记录 | 删除 ch0001_s1.yaml 后没有重建索引 | 运行 build_db.py 重建 SQLite，确保 YAML 和 DB 一致 |
| TC-RO-006 | 批量删了最后 20 个场景文件想重做 | `rm data/novels/nm_xxx/scenes/ch018*` | 重新运行 novel-scenes 拆分对应章节范围 |
...
| TC-RO-013 | 误删 scenes_index.yaml（倒排索引） | `rm data/novels/nm_xxx/scenes_index.yaml` | 运行 build-index 重建 |
| TC-RO-014 | 误删 scenes_manifest.yaml（场景清单） | `rm data/novels/nm_xxx/scenes_manifest.yaml` | 运行 build-index 重建 |
| TC-RO-015 | 误删 plot_index.yaml（全局剧情索引） | `rm data/plot_index.yaml` | 运行 build-index 对任一小说重建，会自动聚合全局 |
| TC-RO-016 | 误删 character_index.yaml | `rm data/character_index.yaml` | 同上，build-index 会自动聚合 |
| TC-RO-017 | 误删 chapter_index.yaml | `rm data/novels/nm_xxx/chapter_index.yaml` | 重新运行 source-format 生成 |
| TC-RO-018 | 同时删了 material.db 和 scenes_index.yaml | 两个文件都删了 | 先 build-index 生成 YAML 索引，再 build_db.py 重建 SQLite |
...
| TC-RO-023 | 误删 characters.yaml | `rm data/novels/nm_xxx/characters.yaml` | git checkout 或重新运行 novel-characters |
...
| TC-RO-026 | 同时误删 outline + characters + tags | 三个分析文件一起被删 | git checkout 或重新运行 pipeline-analyze |
| TC-RO-027 | 清空整个小说文件夹后在回收站找回 | 从回收站恢复文件夹 | 检查文件完整性，运行 validate_yaml 校验场景 |
...
| TC-RO-030 | 误删 scripts/core/ 下的脚本文件 | `rm scripts/core/search.py` | git checkout 恢复，这些脚本在版本控制中 |
...
| TC-RO-034 | git reset --hard 丢失未提交的场景 | 大量新拆分的场景未提交就 reset 了 | 用 `git reflog` 找回，或重新运行场景拆分 |
| TC-RO-035 | 误操作 git clean -fd 清除未追踪文件 | 新生成但未 git add 的文件被清除 | git reflog 或重新生成 |
| TC-RO-036 | 把 A 小说的 scenes 文件夹覆盖到了 B 小说 | `cp -rf novels/A/scenes/ novels/B/scenes/` | 从 git 恢复 B 的 scenes，重新 build-index |
| TC-RO-037 | 全局替换操作把 YAML 中的某个标签值改错了 | IDE 全局替换影响了多个文件 | git diff 查看改动范围，git checkout 恢复受影响文件 |
| TC-RO-038 | 误用 sed 批量修改了场景 YAML 的 tension 字段 | `sed -i 's/tension: 5/tension: 1/g' *.yaml` | git checkout 恢复，运行 validate_yaml 校验 |
| TC-RO-039 | 磁盘空间满了导致写入不完整 | YAML 文件写到一半磁盘满了 | 清理磁盘空间，删除不完整文件，重新执行该批次 |
| TC-RO-040 | 在 pipeline 执行过程中强制关机 | 正在写场景文件时断电 | 开机后检查最后一批场景的完整性，continue 恢复 |
...
| ID | 操作场景 | 错误示例 | 预期行为 |
|----|---------|---------|---------|
| TC-RO-041 | 手动编辑场景 YAML 时缩进不一致 | 混用 Tab 和空格 | validate_yaml 报错定位到具体行 |
...
| TC-RO-048 | 多了一个冒号导致嵌套层级错误 | `scene_type: : 对决` | YAML 解析错误 |
| TC-RO-049 | 遗漏了必填字段 id | 编辑时误删了 id 行 | validate_yaml 报缺少必填字段 |
| TC-RO-050 | 遗漏了必填字段 chapter | 编辑时误删了 chapter 行 | validate_yaml 报缺少必填字段 |
| TC-RO-051 | id 格式不对（不是 ch{NNNN}_s{NN}） | `id: scene_15` | validate_yaml 报 ID 格式错误 |
...
| TC-RO-065 | tags.yaml 中维度名拼写错误 | `scnee_type` 代替 `scene_type` | 使用该维度的校验全部失败 |
| TC-RO-066 | tags.yaml 格式被破坏（如缺少 values 键） | `scene_type: {description: "场景类型"}` | 系统报错，标签校验无法进行 |
...
| TC-RO-069 | chapter_index.yaml 中章节名有拼写错误 | `第1张 开始` 代替 `第1章 开始` | 场景中 chapter 字段与 chapter_index 不匹配 |
...
| ID | 操作场景 | 错误示例 | 预期行为 |
|----|---------|---------|---------|
| TC-RO-071 | LLM 生成的场景 YAML 包含多余的 Markdown 标记 | 开头有 ```yaml，结尾有 ``` | 解析时自动去除 Markdown 围栏 |
| TC-RO-072 | LLM 生成的 summary 超长（>500 字） | summary 写成了一整段分析 | 应有长度校验，提示截断或重新生成 |
| TC-RO-073 | LLM 生成了嵌套格式而非扁平格式 | 使用了旧的 content/people/emotion 分组 | build_db.py 的 _flatten_scene() 兼容处理 |
| TC-RO-074 | LLM 在 title 中使用了编号 | `title: 场景3` | quality_audit 检测到并告警 |
| TC-RO-075 | LLM 给所有场景都打了相同的 emotion 标签 | 10 个场景全是 `emotion: [平静]` | quality_audit 检测到标签多样性不足 |
| TC-RO-076 | LLM 给出了 tags.yaml 中不存在的标签值 | `scene_type: [超自然]`（不在合法值中） | validate_yaml 检测到非法标签值 |
| TC-RO-077 | LLM 遗漏了 characters 字段 | 场景 YAML 中无 characters 列表 | validate_yaml 报缺少必填字段 |
| TC-RO-078 | LLM 输出的 YAML 有语法错误 | 例如列表缩进不对 | 写入前应先 yaml.safe_load 验证 |
| TC-RO-079 | LLM 把 tension 写成了中文 | `tension: 三` | validate_yaml 报类型错误 |
| TC-RO-080 | LLM 输出了空的场景 YAML | 只有注释没有内容 | 跳过空文件，不崩溃，报告问题 |
...
| TC-RO-084 | novel-characters 执行一半断网 | 网络中断 | 检查 characters.yaml 是否完整，不完整则删除重做 |
...
| TC-RO-087 | novel-scenes 在第 15 批中断 | 长时间运行后 token 耗尽 | continue 模式从第 16 批恢复 |
| TC-RO-088 | novel-scenes 在第 30 批时开新对话 | 按建议每 30 批分段 | 新对话 continue 从第 31 批开始 |
| TC-RO-089 | novel-scenes 在 all 模式中突然退出 | 浏览器崩溃 | meta.yaml 记录了最后完成的批次，continue 恢复 |
...
| TC-RO-095 | full pipeline 在 scenes 阶段中断后开新对话 | 对话超长后换新对话 | /novel-pipeline continue [id] 从上次位置恢复 |
| TC-RO-096 | 同时运行两个 pipeline 处理同一本书 | 误开两个终端 | 应检测到冲突并阻止，或至少不破坏数据 |
| TC-RO-097 | 同时运行两个 pipeline 处理不同的书 | 并行处理两本小说 | 各自独立运行，互不影响 |
| TC-RO-098 | pipeline 执行到一半手动修改了场景文件 | 在 novel-scenes 运行中编辑了已生成的 YAML | 不影响后续批次，但修改可能在 build-index 时被检测到 |
| TC-RO-099 | pipeline 执行到一半手动修改了 meta.yaml 的 status | 把 status 从 outlined 改成了 raw | pipeline 可能回退到不正确的阶段 |
| TC-RO-100 | pipeline 执行到一半删除了 chapter_index.yaml | 场景拆分依赖此文件 | novel-scenes 无法确定章节名，报错停止 |

### 3.2 恢复场景验证（TC-RO-101 ~ TC-RO-120）

| ID | 操作场景 | 验证步骤 | 预期结果 |
|----|---------|---------|---------|
| TC-RO-101 | 中断恢复后检查场景文件连续性 | 查看 scenes/ 目录下文件编号是否连续 | 无缺失、无重复 |
| TC-RO-102 | 中断恢复后检查 meta.yaml 进度 | 读取 meta.yaml 中的 pipeline 相关字段 | 进度信息准确反映实际完成状态 |
| TC-RO-103 | 恢复后的场景与中断前的场景标签风格一致 | 对比中断前后的场景标签分布 | 风格连贯，不因换对话而突变 |
| TC-RO-104 | 恢复后 build-index 能正确处理所有场景 | 运行 build-index | 索引覆盖所有场景，包括中断前后的 |
| TC-RO-105 | 恢复后搜索能找到所有已处理的场景 | 搜索某个标签值 | 中断前后的场景都能被搜索到 |
| TC-RO-106 | 手动删除最后 5 个场景后 continue | 删了几个然后恢复 | 从删除后的位置重新拆分，不跳过 |
| TC-RO-107 | 修改了 meta.yaml 中的批次记录后 continue | 手动把 last_batch 改小 | 从修改后的位置重新拆分，可能重复 |
| TC-RO-108 | continue 时发现 source.txt 被修改过 | 有人编辑了原文 | 新场景基于修改后的原文拆分，可能与旧场景不一致 |
| TC-RO-109 | continue 时发现 tags.yaml 新增了标签值 | 中间更新了标签字典 | 新场景可以使用新标签值 |
| TC-RO-110 | continue 时发现 chapter_index.yaml 被修改 | 中间修正了章节名 | 新场景使用修正后的章节名，已有场景不变 |
| TC-RO-111 | 全部场景拆完后再 continue | 已经完成所有章节 | 系统提示已完成，不做重复工作 |
| TC-RO-112 | stage 模式指定执行 pipeline-scenes | `/novel-pipeline stage [id] scenes` | 仅执行场景拆分+索引阶段 |
| TC-RO-113 | stage 模式执行 finalize 但 scenes 未完成 | 场景拆分未完就跑精调 | 系统提示前置阶段未完成 |
| TC-RO-114 | quick 模式完成后切换到 full 模式 | 快速骨架后想补全 | 从 scenes 阶段继续，不重复 analyze |
| TC-RO-115 | pipeline 完成后重新运行 full | 已完成的书再跑一遍 | 全部覆盖重新生成，或提示已完成可选择跳过 |
| TC-RO-116 | 中断后手动补了几个场景文件再 continue | 自己写了场景 YAML 放进去 | continue 跳过已有编号，继续后续章节 |
| TC-RO-117 | 场景文件编号有空洞后 build-index | ch0010 不存在但 ch0011 存在 | build-index 正常处理，不因编号不连续而失败 |
| TC-RO-118 | 修改了一个场景的标签后没有重建索引 | 改了标签但没 build-index | SQLite 搜索结果与 YAML 不一致直到重建 |
| TC-RO-119 | 批量修改场景标签后重建索引 | 改了 50 个场景的 emotion 标签 | build-index 全部更新，搜索结果正确 |
| TC-RO-120 | 添加新场景文件后不重建索引就搜索 | 手动写了新场景 | 新场景在 SQLite 中搜不到，YAML 索引中也没有 |
...
| ID | 操作场景 | 错误示例 | 修正方式 |
|----|---------|---------|---------|
| TC-RO-121 | 场景标签值拼写错误 | `emotion: [燃燃]`（多了一个字） | 手动修正 YAML，重新 build-index |
| TC-RO-122 | 标签维度名写错 | `scence_type` 代替 `scene_type` | 手动修正，validate_yaml 会检测到 |
...
| TC-RO-127 | characters 列表人名写错 | `characters: [张三三]`（正确应为张三） | 人物搜索找不到该场景，需手动修正 |
| TC-RO-128 | chapter 字段手写不从 chapter_index 拷贝 | `chapter: 第一章 开始`（index 中是"第1章 开始"） | validate_yaml 检测章节名不匹配 |
| TC-RO-129 | 多个场景 YAML 中使用了同一个 id | 两个文件都是 `id: ch0001_s1` | build-index 检测到重复 ID 报错 |
| TC-RO-130 | 场景文件名与内部 id 不匹配 | 文件名 ch0001_s1.yaml 但 id 写成 ch0002_s1 | validate_yaml 检测到不一致 |
...
| TC-RO-133 | 场景 summary 内容与实际章节完全不符 | summary 描述的是另一个场景 | 手动修正 summary，参照 source.txt 对应行号 |
| TC-RO-134 | 场景 title 含有非法字符 | `title: 场景/杀戮`（斜杠可能影响文件名） | 手动修正，避免使用文件系统不支持的字符 |
| TC-RO-135 | 人物 role 字段写了非标准值 | `role: 小角色`（应为 protagonist/antagonist/supporting 等） | 手动修正为标准值 |
| TC-RO-136 | 人物 archetype 写了 tags.yaml 中没有的原型 | `archetype: 超级战士` | 检查是否需要 tag-add 或修正为已有值 |
...
| TC-RO-145 | pipeline_status.json 中的进度记录被手动篡改 | 把某本书的状态从 scenes 改成 complete | pipeline 跳过后续步骤，导致数据不完整 |
...
### 4.3 场景批次修正（TC-RO-156 ~ TC-RO-170）

| ID | 操作场景 | 操作步骤 | 预期结果 |
|----|---------|---------|---------|
| TC-RO-156 | 发现第 5 批场景质量太差，想整批重做 | 删除 ch0041-ch0050 对应的所有场景文件 | 重新运行 novel-scenes 指定该章节范围 |
| TC-RO-157 | quality_audit 报告某批次标签雷同 | 审查后确认确实需要重做 | 删除该批文件，重新拆分 |
| TC-RO-158 | 手动修改了 3 个场景的标签后想验证 | 编辑了 YAML 文件 | 运行 validate_yaml 校验，通过后 build-index |
| TC-RO-159 | 场景拆分粒度太粗，一章只有 1 个场景 | 想把一个场景拆成两个 | 手动创建新的场景文件，调整 ID 编号 |
| TC-RO-160 | 场景拆分粒度太细，一章有 10 个场景 | 想合并几个场景 | 删除多余文件，合并内容到保留的文件中 |
| TC-RO-161 | 两本小说的场景 ID 意外冲突 | 不同小说生成了相同的 ch0001_s1 | 场景 ID 应包含 material_id 前缀，确认不冲突 |
| TC-RO-162 | 修改了一个场景的 chapter 字段后索引对不上 | 改了章节名但 chapter_index 没更新 | 保持 chapter 字段与 chapter_index.yaml 一致 |
| TC-RO-163 | 批量修改了场景的 pov 字段 | 发现整本书 pov 标记错误 | 全局替换后运行 validate_yaml + build-index |
| TC-RO-164 | 在已完成的书中插入了新章节 | 作者更新了原文，中间新增了一章 | 需要重新 source-format + novel-scenes（受影响范围） |
| TC-RO-165 | 发现 source.txt 和 source.raw.txt 不同步 | 手动编辑了 source.txt 但没更新 raw | 二者可以不同（raw 是原始备份），但需记住哪个是权威 |
| TC-RO-166 | 多人同时编辑同一个场景文件 | A 和 B 都改了 ch0015_s2.yaml | git merge 处理冲突或约定编辑锁 |
| TC-RO-167 | refine 后想回退到 refine 前的版本 | 精调结果不满意 | git checkout 到 refine 前的提交 |
| TC-RO-168 | novel-stats 统计数据与手动数不一致 | stats 显示 285 个场景，实际有 283 个文件 | 检查 stats 的数据来源，是否有空文件或格式错误 |
...
| TC-RO-178 | 打开 MaterialDetail 但 stats.html 很大（>5MB） | 场景非常多的书 | 加载时有 loading 提示，不卡死 |
...
| TC-RO-182 | 刷新 SceneSearch 结果页 | 搜索后刷新浏览器 | 搜索条件丢失，页面恢复到初始状态 |
...
| TC-RO-194 | 搜索框输入 SQL 关键字 | `DROP TABLE scenes` | 不执行 SQL（注入防御） |
...
| TC-RO-199 | CharacterSearch 输入不存在的人物名 | 搜索"王五"但没有这个人物 | 返回空结果，提示"未找到" |
| TC-RO-200 | SceneSearch 选择了所有标签维度 | 每个维度都选了值 | 返回严格满足所有条件的结果（可能为空） |
| TC-RO-201 | 搜索结果中点击某个场景查看详情 | 点击搜索结果卡片 | 跳转到对应素材详情页或展开详情 |
...
| TC-RO-207 | 后端搜索 API 返回空的 JSON | `{"scenes": []}` | 前端显示"无结果"而非空白 |
...
### 6.1 search.py 命令行操作（TC-RO-221 ~ TC-RO-235）

| ID | 操作场景 | 命令 | 预期行为 |
|----|---------|------|---------|
| TC-RO-221 | 不带参数运行 search.py | `python scripts/core/search.py` | 显示帮助信息，不报错 |
| TC-RO-222 | scene 搜索不带任何条件 | `python scripts/core/search.py scene` | 提示至少提供一个条件或返回全部 |
| TC-RO-223 | scene 搜索使用非法的维度名 | `python scripts/core/search.py scene --moood 开心` | 报错提示无效参数 |
| TC-RO-224 | character 搜索不带条件 | `python scripts/core/search.py character` | 返回全部人物或提示提供条件 |
| TC-RO-225 | text 搜索使用超长查询文本 | 输入 1000 字搜索文本 | 正常搜索或提示长度限制 |
| TC-RO-226 | search.py 在 material.db 不存在时运行 | 数据库文件缺失 | 报错提示数据库不存在，建议运行 build_db.py |
| TC-RO-227 | search.py 在 material.db 为空时运行 | 数据库存在但无数据 | 返回空结果 |
| TC-RO-228 | search.py 结果输出 JSON 格式 | 使用 --format json 参数 | 输出合法 JSON |
| TC-RO-229 | search.py 使用 --limit 0 | limit 设为 0 | 返回空或提示参数无效 |
| TC-RO-230 | search.py 使用 --limit 负数 | `--limit -5` | 报错提示参数无效 |
| TC-RO-231 | search.py 使用 --tension-min 和 --tension-max | 范围查询 | 返回范围内的场景 |
| TC-RO-232 | search.py 使用 --tension-min > --tension-max | 无效范围 | 返回空或报错提示范围无效 |
| TC-RO-233 | 多次连续运行 search.py | 快速运行多次搜索 | 每次都返回正确结果，无状态残留 |
| TC-RO-234 | 在错误的工作目录下运行 search.py | 不在项目根目录运行 | 报错提示找不到数据库或自动定位 |
...
| TC-RO-237 | 有场景 YAML 格式错误时运行 build_db.py | 某个 YAML 文件解析失败 | 跳过损坏文件继续处理，报告哪些文件有问题 |
...
| TC-RO-241 | build_db.py 处理含旧格式（嵌套）场景 | 新旧格式混合 | _flatten_scene() 正确处理两种格式 |
| TC-RO-242 | build_db.py 处理超大数据量（10000+ 场景） | 多本大书 | 运行完成不超时，数据正确 |
| TC-RO-243 | 磁盘空间不足时 build_db.py | 空间不够创建 SQLite | 报错提示磁盘空间不足 |
| TC-RO-244 | build_db.py 处理 characters.yaml 格式变化 | 人物数据结构变更 | 兼容新旧格式，不丢失人物数据 |
...
| ID | 操作场景 | 命令/操作 | 预期行为 |
|----|---------|---------|---------|
| TC-RO-246 | validate_yaml 校验一个完全合规的场景 | 校验正确的 YAML | 通过校验，无错误报告 |
| TC-RO-247 | validate_yaml 校验缺少必填字段的场景 | 缺少 id 字段 | 报告缺失的必填字段名 |
| TC-RO-248 | validate_yaml 校验有非法标签值的场景 | `emotion: [绝望]` 但 tags.yaml 中没有 | 报告非法值和对应维度 |
| TC-RO-249 | validate_yaml 校验章节名不匹配的场景 | chapter 与 chapter_index 不一致 | 报告不匹配的 chapter 值 |
| TC-RO-250 | validate_yaml 校验空 scenes 目录 | 目录下无文件 | 提示无场景文件可校验 |
| TC-RO-251 | quality_audit 审计标签多样性正常的批次 | 审计第 1-10 章 | 审计通过，无告警 |
| TC-RO-252 | quality_audit 审计标签雷同的批次 | 10 个场景标签完全相同 | 报告雷同率和具体场景 ID |
| TC-RO-253 | quality_audit 审计 title 为编号形式 | `title: 场景1` | 报告使用了编号形式的 title |
| TC-RO-254 | quality_audit --batch 参数传了累积范围 | `--batch 1-200` 而非 `--batch 181-200` | 应该只审计 181-200 而非全部 |
| TC-RO-255 | quality_audit 批次范围超出实际场景数 | `--batch 301-320` 但只有 283 个场景 | 报告范围超出，只审计存在的场景 |
...
| TC-RO-260 | build_scene_index.py 处理嵌套和扁平混合 | 同一本书两种格式 | 全部正确索引 |
...
| TC-RO-264 | 搜索 API 传递非法 JSON body | `POST /api/v1/search {invalid json}` | 422 Unprocessable Entity |
| TC-RO-265 | 搜索 API 传递空 body | `POST /api/v1/search {}` | 返回空结果或提示条件缺失 |
