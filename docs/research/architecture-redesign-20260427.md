...
| plot_index 存在但几乎为空 | 需要真正可用的跨书索引 |
...
CREATE TABLE chapters (
    ...
    summary_embedding vector(1536),      -- pgvector 向量
    word_count INTEGER,
...
CREATE TABLE novels (
    ...
    premise_embedding vector(1536),      -- pgvector 向量
...
CREATE TABLE characters (
    ...
    moral_spectrum TEXT,
    arc_summary TEXT,
    arc_summary_embedding vector(1536),
...
1. **pgvector 向量维度**：使用 OpenAI embedding（1536维）还是本地模型（768/1024维）？
2. **中文全文搜索**：PostgreSQL 原生分词对中文支持有限，是否需要集成 jieba/pg_jieba？
3. **Embedding 服务选择**：本地部署（省钱但慢）还是调用 API（快但有成本）？
4. **钩子处理**：钩子埋设与收束是否需要单独的数据结构？（这是唯一可能需要额外标注的内容）
5. **版本控制**：YAML 文件是否仍作为 Source of Truth 保留，还是全面转向数据库？
