...
## chapter_insights 精调全部章节耗时过长

### 问题
不清楚 chapter_insights 的用途，且 insights 阶段对所有章节进行精调，耗时过长。

### 解决
增加前置导航输出预算配置，控制 insights 处理范围，避免对所有章节执行精调。
...
### 解决
接入报告 sink，实现原子写入机器报告与 Markdown 报告。
...
### 解决
实现分层世界观产物写入，增加题材维度路由和响应归一化，提升世界观分析质量。
...
## --window 与 evaluate.py 绑定设计问题

### 问题
使用 --window 强制单章分析时，evaluate.py 和 --window 绑在一起，设计不合理。
...
### 问题
运行 pipeline full 遇到同名小说时直接重复分析，没有询问用户。

### 解决
拦截同名小说重复完整分析，发现同名时先询问用户是否重新分析。

### 相关文件
- src/novel_material/pipeline/preprocess.py
