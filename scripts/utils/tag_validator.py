...
原有文件读取方式已废弃，改用 scripts/tags/validate.py 提供的数据库校验。
"""
import sys
...
from scripts.tags.validate import validate_tag, validate_tags_batch, check_dimension_usage
...
def suggest_expand(dimension: str, new_tag: str) -> None:
    ...
    if not canonical:
        ...
        print(f"如需添加，请使用:")
        print(f"  python scripts/tags/manage.py add {dimension} '{new_tag}' common")
        print(f"或运行审核流程:")
        print(f"  python scripts/tags/review.py list")

    # 数据库统计已不再需要手动读取文件
    # 标签数量可通过 manage.py stats 查看
...
def flatten_tags(dim_data) -> set:
    ...
    请使用 scripts/tags/validate.py 中的 validate_tags_batch()
    """
...
def build_synonym_reverse(tags_dict: dict) -> dict:
    ...
    请使用 scripts/tags/validate.py 中的 validate_tag()
    """
