...
Covers gaps NOT in the existing test_validate_yaml.py:
  ...
  - cmd_scene() / cmd_meta() / cmd_all() — command-level orchestration
  - _flatten_scene() — additional nested-to-flat edge cases
  - validate_scene() — boundary conditions (tension exactly 1/5, None values)
"""
...
TAGS_DICT_FIXTURE = {
    "scene_type": {"description": "", "values": ["对决", "日常", "回忆"]},
...
def _make_valid_scene():
    return {
        "id": "test_s1", "chapter": "第1章 测试", "title": "好标题",
        "summary": "摘要",
        "tension": 3,
...
# ── _flatten_scene (extra cases) ──────────────────────────────────────


class TestFlattenSceneExtended:
    def test_scene_id_renamed_to_id(self):
        raw = {"scene_id": "s1", "chapter": "ch1", "title": "t", "summary": "s"}
        flat = _flatten_scene(raw)
        assert flat["id"] == "s1"
        assert "scene_id" not in flat

    def test_tension_extracted_from_nested_emotion(self):
        raw = {
            "id": "s1", "chapter": "c", "title": "t", "summary": "s",
    ...
    def test_preserves_flat_format(self):
        ...
        assert flat["id"] == scene["id"]
        assert flat["scene_type"] == scene["scene_type"]


# ── validate_scene (boundary cases) ──────────────────────────────────


class TestValidateSceneBoundary:
    @pytest.fixture()
    ...
    def test_tension_exactly_1_valid(self, tmp_path, tags_dict):
        ...
        _write_yaml(scene_path, scene)
        errors = validate_scene(scene_path, tags_dict, None)
    ...
    def test_top_level_not_dict(self, tmp_path, tags_dict):
        scene_path = tmp_path / "scene.yaml"
        _write_yaml(scene_path, ["not", "a", "dict"])
        errors = validate_scene(scene_path, tags_dict, None)
    ...
    def test_chapter_mismatch_detected(self, tmp_path, tags_dict):
        scene = _make_valid_scene()
        scene["chapter"] = "不存在的章节"
        scene_path = tmp_path / "scene.yaml"
        _write_yaml(scene_path, scene)
        chapter_titles = {"第1章 测试", "第2章 进展"}
        errors = validate_scene(scene_path, tags_dict, chapter_titles)
        assert any("章节名不匹配" in e for e in errors)


# ── cmd_scene / cmd_meta / cmd_all ────────────────────────────────────
...
class TestCommandFunctions:
    ...
    def setup_material(self, tmp_path, monkeypatch):
        ...
        scene = _make_valid_scene()
        _write_yaml(scenes / "ch0001_s1.yaml", scene)
        return mid

    def test_cmd_scene_pass(self, setup_material):
        assert cmd_scene(setup_material) == 0

    def test_cmd_scene_no_scenes_dir(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / "data" / "novels" / "no_scenes").mkdir(parents=True)
        assert cmd_scene("no_scenes") == 1

    def test_cmd_scene_with_pattern(self, setup_material):
        assert cmd_scene(setup_material, "ch0001") == 0
