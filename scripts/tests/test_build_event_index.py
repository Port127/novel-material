"""Tests for build_event_index.py."""
...
from build_event_index import build_index, _as_list
...
class TestBuildIndex:
    def test_builds_outputs(self, novel_env):
        ...
        index_path = novel_dir / "events_index.yaml"
        manifest_path = novel_dir / "events_manifest.yaml"
        ...
        assert "event_type" in index
        assert "character" in index

        with open(manifest_path, "r", encoding="utf-8") as f:
            manifest = yaml.safe_load(f)
        assert len(manifest["events"]) == 10
    ...
    def test_updates_meta(self, novel_env):
        ...
        assert meta["pipeline"]["manifest_events"] == 10