...
from validate_yaml import validate_event, validate_yaml_parseable, _flatten_event, load_tags_dict


def _write_event(path, data):
...
def _make_valid_event():
    return {
        "id": "test_ev0001", "chapter": "第1章 起始", "title": "黎明之战",
        "summary": "精彩的战斗事件",
...
class TestValidateEvent:
    def test_valid_event(self, novel_env):
        _, data_dir, novel_dir, _ = novel_env
        tags_dict = load_tags_dict()
        event_path = novel_dir / "events" / "ev0001.yaml"
        errs = validate_event(event_path, tags_dict, None)
        assert errs == []
    ...
    def test_missing_required_field(self, tmp_path, novel_env):
        ...
        del event["id"]
        event_path = tmp_path / "bad_event.yaml"
        _write_event(event_path, event)
        errs = validate_event(event_path, None, None)
        assert any("id" in e for e in errs)

    def test_invalid_tension(self, tmp_path):
        event = _make_valid_event()
        event["tension"] = 8
        event_path = tmp_path / "bad_tension.yaml"
        _write_event(event_path, event)
        errs = validate_event(event_path, None, None)
    ...
    def test_bad_title(self, tmp_path):
        event = _make_valid_event()
        event["title"] = "事件1"
        event_path = tmp_path / "bad_title.yaml"
        _write_event(event_path, event)
        errs = validate_event(event_path, None, None)
    ...
    def test_illegal_tag_value(self, tmp_path, novel_env):
        ...
        tags_dict = load_tags_dict()
        event = _make_valid_event()
        event["event_type"] = ["不存在的标签"]
        event_path = tmp_path / "illegal_tag.yaml"
        _write_event(event_path, event)
        errs = validate_event(event_path, tags_dict, None)
    ...
    def test_chapter_name_mismatch(self, tmp_path):
        event = _make_valid_event()
        event_path = tmp_path / "mismatch.yaml"
        _write_event(event_path, event)
        chapter_titles = {"第2章 别的标题"}
        errs = validate_event(event_path, None, chapter_titles)
        assert any("章节名不匹配" in e for e in errs)

    def test_unparseable_yaml(self, tmp_path):
        bad_file = tmp_path / "bad.yaml"
        bad_file.write_text(": :\n  - [invalid yaml{{{", encoding="utf-8")
        errs = validate_event(bad_file, None, None)
        assert any("YAML" in e for e in errs)

    def test_empty_file(self, tmp_path):
        empty_file = tmp_path / "empty.yaml"
        empty_file.write_text("", encoding="utf-8")
        errs = validate_event(empty_file, None, None)
