...
Covers: search_scenes, search_characters, search_text, show_stats, get_conn, argparse.
"""
...
def _build_db(db_path: Path):
    ...
    conn.executescript("""
        ...
        CREATE INDEX idx_st_dv ON scene_tags(dimension, value);
        CREATE INDEX idx_sc_name ON scene_characters(character_name);
    """)
    ...
    scenes = [
        ("s1", mid, "第1章", "黎明之战", "主角与宿敌在黎明决战", 4, "快", "第三人称限制", "以弱胜强", "正义", "发展", "个人"),
        ("s2", mid, "第2章", "温暖回忆", "回忆师父的教导", 2, "慢", "第一人称", "势均力敌", "灰色", "开端", "个人"),
        ("s3", mid, "第3章", "最终对决", "正邪最终大战", 5, "快", "全知", "以弱胜强", "正义", "高潮", "国家"),
        ("s4", mid, "第4章", "告别", "主角与朋友告别离去", 3, "中", "第三人称限制", "势均力敌", "正义", "结局", "个人"),
    ]
    conn.executemany("INSERT INTO scenes VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", scenes)

    tag_rows = [
        ("s1", mid, "scene_type", "对决"), ("s1", mid, "emotion", "燃"),
        ("s1", mid, "conflict", "人与人"), ("s1", mid, "reader_effect", "爽感"),
        ("s2", mid, "scene_type", "回忆"), ("s2", mid, "emotion", "温暖"),
        ("s2", mid, "emotion", "悲伤"),
        ("s3", mid, "scene_type", "对决"), ("s3", mid, "emotion", "燃"),
        ("s3", mid, "conflict", "人与命运"), ("s3", mid, "reader_effect", "催泪"),
        ("s4", mid, "scene_type", "日常"), ("s4", mid, "emotion", "悲伤"),
    ]
    conn.executemany("INSERT INTO scene_tags (scene_id, material_id, dimension, value) VALUES (?,?,?,?)", tag_rows)
...
class TestSearchScenes:
    ...
    def test_single_tag_filter(self, search_env, capsys):
        mod, _ = search_env
        args = self._make_args(scene_type="对决")
        mod.search_scenes(args)
        out = capsys.readouterr().out
        parsed = yaml.safe_load(out)
        assert parsed["total"] == 2
        ids = {r["scene_id"] for r in parsed["results"]}
        assert ids == {"s1", "s3"}

    def test_multi_tag_intersection(self, search_env, capsys):
        mod, _ = search_env
        args = self._make_args(scene_type="对决", conflict="人与命运")
        mod.search_scenes(args)
        out = capsys.readouterr().out
        parsed = yaml.safe_load(out)
        assert parsed["total"] == 1
        assert parsed["results"][0]["scene_id"] == "s3"
    ...
    def test_character_filter(self, search_env, capsys):
        ...
        parsed = yaml.safe_load(out)
        assert parsed["total"] == 1
        assert parsed["results"][0]["scene_id"] == "s2"
    ...
    def test_no_match_prints_zero(self, search_env, capsys):
        mod, _ = search_env
        args = self._make_args(scene_type="不存在的类型")
        mod.search_scenes(args)
        out = capsys.readouterr().out
    ...
    def test_relaxation_on_impossible_intersection(self, search_env, capsys):
        ...
        mod, _ = search_env
        args = self._make_args(scene_type="回忆", conflict="人与命运")
        mod.search_scenes(args)
        out = capsys.readouterr().out
    ...
    def test_score_sorting(self, search_env, capsys):
        mod, _ = search_env
        args = self._make_args(scene_type="对决", emotion="燃")
        mod.search_scenes(args)
        out = capsys.readouterr().out
        parsed = yaml.safe_load(out)
    ...
    def test_query_echoed_in_output(self, search_env, capsys):
        mod, _ = search_env
        args = self._make_args(scene_type="对决", character="张三")
        mod.search_scenes(args)
        out = capsys.readouterr().out
        parsed = yaml.safe_load(out)
        assert parsed["query"]["scene_type"] == "对决"
...
class TestShowStats:
    ...
    def test_top_scene_types(self, search_env, capsys):
        mod, _ = search_env
        mod.show_stats(types.SimpleNamespace())
        out = capsys.readouterr().out
        assert "top_scene_types:" in out
        assert "对决" in out
...
class TestMainArgparse:
    def test_scene_subcommand(self, search_env, monkeypatch, capsys):
        mod, _ = search_env
        monkeypatch.setattr(sys, "argv", ["search.py", "scene", "--scene-type", "对决"])
        mod.main()
        out = capsys.readouterr().out
        parsed = yaml.safe_load(out)
        assert parsed["total"] >= 1
