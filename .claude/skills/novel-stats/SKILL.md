...
从 `outline/hooks_network.yaml` 采集钩子数据：
...
**数据来源**：
- 从 `outline/hooks_network.yaml` 的 `stats` 读取总体统计
...
从 `characters/_index.yaml` + `relations.yaml` 提取关系网数据，构建图谱：
...
- **nodes**: 从 `characters/_index.yaml` 的 `roster` 提取，附加出场统计
- **edges**: 从 `characters/relations.yaml` 的 `relations` 提取，weight 为共同出场事件数
...
- 人物关系图谱数据来源：characters/_index.yaml + relations.yaml（roster + relations + factions）+ 事件出场统计
- 钩子统计统一处理反转铆合：从 outline/hooks_network.yaml 读取，反转铆合即传统"伏笔"
