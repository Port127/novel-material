...
|----------|------|------|
| 书名/作者名 | 素材级匹配 | "三体""刘慈欣" |
| 角色名 | 人物索引 + 场景索引 | "叶文洁""吕树" |
| 场景需求描述 | 直接路由到 `/material-search-scene` | "恋人吵架""弱者反杀" |
...
返回匹配的小说列表，并标注匹配的风格维度。如果后续还有场景级检索需求，在这些小说范围内进行。
...
- 如关键词是角色名 → 查 `scenes_index.yaml` 的 `character` 维度
- 如关键词可映射到标签值 → 查对应维度
- 命中后只读取候选 scene YAML 获取详情

**Level 3 — 遍历场景文件**（兜底）：
- 仅在无索引且无清单时使用
- 分批遍历 `data/novels/*/scenes/*.yaml`，匹配 `title`, `summary`, `characters`
...
- 如果明显是场景需求，直接路由到 `/material-search-scene`，不做关键词匹配
- 返回原文定位信息（`text_range`），方便用户追溯
- 场景级检索必须遵循索引优先原则，避免无谓遍历全部场景文件
